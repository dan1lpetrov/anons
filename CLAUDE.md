...
### Reseller conditions (commission / discount) and site-wide currency

Each campaign (`sale_events` row) carries `buyer_commission_percent` (default 10) and `additional_discount_percent` (default 0) — different campaigns can have different margins. Display currency is **not** per-campaign: it's a single `site_settings` row (`api/_lib/siteSettings.ts`, `getSiteCurrency`/`setSiteCurrency`), `display_currency` (`'original'` = the scraper's USD, or `'uah'`) + `uah_bank` (`'mono' | 'privat' | null`) + `uah_rate`, because a shopper browsing the catalog can't sensibly see some products in $ and others in ₴. `null` bank means "власний курс" (custom) — `uah_rate` is always admin-typed for that case; `mono`/`privat` (`api/_lib/exchangeRates.ts`'s `NAMED_BANKS`, the only two with public no-auth rate APIs) always **re-fetch a fresh rate right at save time**, ignoring whatever the client showed — "post with the actual current rate," not a possibly-stale one sitting in a form. Edited from `/admin/sales`'s top panel (`POST /api/sales` with `{ global: true, ... }`, resolved by `resolveSiteCurrency`).

The markup formula (`applySaleMarkup` in `api/_lib/pricing.ts` — the only place it lives; nothing client-side duplicates it anymore) is **sequential**: `basePrice × (1 − discount%) × (1 + commission%)`, then `× rate` if the site's `displayCurrency === 'uah'`. Every `Product`/`ProductColor` carries `basePrice`/`baseOriginalPrice` (the raw scraped price, untouched) alongside the markup-applied `price`/`originalPrice` actually displayed/sold, plus a `currency: 'USD' | 'UAH'` stamp so `formatPrice()` (`src/utils/format.ts`) shows the right symbol — critical mid-sweep (see below), where some products have been repriced under the new currency and others haven't yet.

**Nothing computes price client-side anymore** — `admin-upload.html` sends raw `basePrice`/`baseOriginalPrice`; `api/products.ts`'s `handlePost` looks up the target campaign's commission/discount and the current site currency and calls `repriceProductData` itself before storing. This is also what makes re-uploading already-priced products (via "Завантажити поточні товари", which round-trips `basePrice`) idempotent instead of compounding markup — a product from before `basePrice` existed has none, so `price` itself is treated as the base.

`price`/`originalPrice` are a **cache**, always kept in sync by a repricing sweep, not computed fresh on every `GET /api/products` (deliberate — a per-request join + JS computation over the whole catalog was rejected as unnecessary read-path cost when "recompute on every write, trust the cache on every read" gives the same correctness). Three things trigger a sweep, all going through `repriceProducts` in `api/sales.ts`:
- **Upload**: `api/products.ts` reprices just the uploaded batch.
- **Editing one campaign's commission/discount** (`POST /api/sales` with `id` + a condition field): `repriceCampaignProducts` reprices every product in that campaign.
- **Editing the site-wide currency**: `repriceAllProducts` reprices *every* product across *every* campaign (each keeping its own campaign's commission/discount) — the whole point of moving currency to `site_settings`.

A plain rename/date/pause save (no condition fields in the body) skips repricing entirely — `conditionsProvided` in `handlePost` gates it.

**`price_history` tracks the pre-FX price, not the displayed one** — `basePrice × (1 − discount%) × (1 + commission%)`, computed by calling `repriceProductData` a second time with a synthetic `{ displayCurrency: 'original', uahRate: null }` (see the `NO_FX` constant in `api/sales.ts`, mirrored in `api/products.ts`'s upload path). This is deliberate: a USD/UAH rate drifts constantly on its own, and logging a new history row every time it ticks — for every product on the site — would bury the signal that actually matters (did the base price or the deal terms change) in currency noise. `recordPriceChanges` (`api/_lib/pricing.ts`, shared by both write paths) only inserts when this pre-FX value differs from the last recorded one per `(product_id, color_id)`, so a pure currency-only sweep writes zero history rows even though it updates `data.price` for every product.
...
Data comes from a `GET /api/events?dashboard=1&days=7|30|90` branch bolted onto `api/events.ts` (admin-gated via `requireAdmin`) rather than a new `api/` file — the project is already at Vercel Hobby's 12-serverless-function cap (see "Home screen banners" above for the same constraint hit before). One request returns everything the page renders: period-over-period KPIs (orders, revenue, views, conversion) with a daily sparkline series, a 7-day × 6-hour-bucket activity heatmap, sales ending soon (from `sale_windows`), a per-brand product count, and the top N products by `product_scores.score` with period-scoped view/order counts and discount %. `api/_lib/pricing.ts` holds the `effectivePrice`/`effectiveOriginalPrice`/`bestDiscount` helpers shared between this endpoint and `api/cron/recompute-scores.ts` (previously duplicated there).

The "revenue" KPI required extending `product_events`: it previously had no concept of money or of "one order" at all — a client checkout fired one `'order'` event per **unique product id** (deduped, no quantity/price). `product_events` now also has `order_id TEXT`, `quantity INTEGER`, `unit_price NUMERIC` columns; `handleSubmitOrder` in `src/App.tsx` fires one event **per cart line** (no dedup) carrying that line's `orderId`/`quantity`/`unitPrice`, so the dashboard can do `COUNT(DISTINCT order_id)` for an order count and `SUM(quantity * unit_price)` for revenue. This is a deliberate behavior change from the old dedup-by-product-id event: a product ordered in two sizes in the same order now logs two `'order'` events instead of one, which also feeds `recompute-scores.ts`'s "orders" ranking signal — more cart lines is a legitimate stronger demand signal, not a bug. Rows written before this migration have `unit_price IS NULL`; revenue sums explicitly filter those out rather than treating a null price as 0.
...
`banners` table (`api/_lib/db.ts`) is a small admin-managed CMS — the only hand-curated content in an otherwise fully-scraped catalog. `api/banners.ts`: `GET` is public and returns only `active` rows ordered by `sort_order` (for the storefront carousel); `GET ?all=1` is admin-gated and returns every row (for `/admin/banners`'s management list). `POST`/`DELETE` are admin-gated. Every banner links to exactly one destination, chosen in `/admin/banners` via a required radio (no "none" option): `linkCategoryId` (picked from a `<select>` populated from the categories actually present in `GET /api/products`, since `CategoryId` is a free-form string, not a closed enum — labels come from a local `CATEGORY_LABELS` map mirroring `src/data/categories.ts`), `linkSaleId` (one of the `SaleId` brand enum, offered only from currently-active sales per `GET /api/sales`), or `linkUrl` (any admin-typed `http(s)://` URL, e.g. an external site). `api/banners.ts`'s `handlePost` enforces the mutual exclusion server-side with priority `linkUrl` > `linkSaleId` > `linkCategoryId` if a client somehow sends more than one. Tapping a banner in `BannerCarousel` calls `Home`'s `onSelect`, which — in that same priority order — opens `linkUrl` via `useTelegram`'s `openLink` (Telegram's in-client browser, or `window.open` outside Telegram), routes to `/catalog?brands=<saleId>` for a sale (reusing the existing brand-filter query param from `useCatalogParams`, not a dedicated sale route), or routes to Catalog filtered by category; a legacy banner saved before the link became mandatory (all three null) falls back to the unfiltered Catalog.
